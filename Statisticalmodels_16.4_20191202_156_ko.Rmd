...
What does it mean when an election forecaster tells us that a given candidate has a 90% chance of winning? In the context of the urn model, this would be equivalent to stating that the probability $p>0.5$ is 90%. However, as we discussed earlier, in the urn model p is a fixed parameter and it does not make sense to talk about probability. With Bayesian statistics, we model p

as random variable and thus a statement such as “90% chance of winning” is consistent with the approach.
...
The cystic fibrosis rate is 1 in 3,900 which implies that $Prob(D=1)=0.00025. To answer this question, we wiil use Bayes theorem, which in general tells us that:

$$Pr(A|B)=\frac{Pr(B|A)Pr(A)}{Pr(B)}$$

This equation applied to our problem becomes

$$Pr(D=1|+)=\frac{P(+|D=1)⋅P(D=1)}{Pr(+)} = \frac{Pr(+|D=1)⋅P(D=1)}{Pr(+|D=1)⋅P(D=1)+Pr(+|D=0)Pr(D=0)}$$
...
$$p ∼ N(\mu,\gamma^2) \\ Y\ |\ p ∼ N (p,\sigma^2)$$
...
$$with B = \frac{\sigma^2}{\sigma^2 + \gamma^2}$$
...
$$ E(p \ | \ Y = .450) = B \times .275 + (1-B) \times .450 \\ = .275 + (1-B)(.450-.275)$$ 
$$ B = \frac{.111^2}{.111^2 + .027^2} = 0.944$$
$$ E(p \ | \ Y = .450) ≈ .285$$

We do not show the derivation here, but the standard error can be shown to be:

$$SE(p | y)^2 = \frac{1}{1/\sigma^2+1/\gamma^2} = \frac{1}{1/.111^2+1/.027^2} = 0.00069$$
...
7. Now assume a Bayesian model that sets the prior distribution for Florida’s election night spread $d$ to be Normal with expected value $\mu$ and standard deviation $\gamma$. What are the interpretations of $\mu$ and $\gamma$?

a. $μ$ and $τ$ are arbitrary numbers that let us make probability statements about d.

8. The CLT tells us that our estimate of the spread $\hat d$ has normal distribution with expected value $d$ and standard deviation $\sigma$ calculated in problem 6. Use the formulas we showed for the posterior distribution to calculate the expected value of the posterior distribution if we set $\mu=0$ and $\gamma=0.01$.
...
In a previous section, we genrated these data tables:
...
$$d ∼ N  (\mu, \gamma^2) \ describes \ our  \ best \ guess \ had \ we \ not \ seen \ any \ polling\  data$$
$$\hat X |d ∼ N  (\mu, \gamma^2) \ describes \ randomness \ due\  to \ sampling \ and \ the \  pollster  \ effect$$
...
Now suppose we have $J=6$ data points from $I=5$ different pollsters. To represent this we now need two indexes, one for pollster and one for the polls each pollster takes. We us $X_{i,j}$ with $i$ representing the pollster and $j$ representing the $j$-th poll from that pollster. If we apply the same model, we write:
...
$$\bar X= d+ b+ \frac{1}{N}\sum_{i=1}^NX_{i,j}$$
...
tau <- 0.02
library(tidyverse)
electoral_votes <- results$electoral_votes.x
...
```{r}
cis <- polls %>% 
  mutate(X = (spread + 1)/2, se = 2*sqrt(X*(1-X)/samplesize), lower = spread - qnorm(0.975)*se, upper = spread + qnorm(0.975)*se) %>% 
...
```{r}
cis = cis %>% 
  mutate(result = ifelse(actual_spread >lower & actual_spread < upper, 1 ,0))

sum(cis$result)/length(cis$result)
```
...
hits <- cis %>% 
  ...
  group_by(pollster) %>% 
  filter(n() > 5) %>% 
  summarize(proportion = mean(hit), n = n( )) %>%
...
```{r}
states %>% ggplot(aes(state, proportion)) + 
...
$$Z=\frac{X-d}{s/\sqrt{N}}$$
By substituting $\sigma$ with $s$ we introduce some variability. The theory tells us that $N$ follows a t-distribution with $N-1$ *degrees of freedom*. The degrees of freedom is a parameter that controls the variability via fatter tails:
