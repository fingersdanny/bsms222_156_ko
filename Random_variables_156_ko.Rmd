...
ifelse(sample(beads, 1) == "blue", 1, 0)
ifelse(sample(beads, 1) == "blues", 1, 0)
```
...
We are going to define a random variable `S`
...
The 1,000 outcomes from 1,000 people playing are independent draws from this urn. If red comes up, the gambler wins and the casino loses a dollar, so we draw a -$$\text{$}$$ 1. Otherwise, the casino wins a dollar and we draw a $$\text{$}$$1. To construct our random variable S, we can use this code:
...
$F(a) = Pr(S≤a)$, then we will be able to answer any question related to the probability of events defined by our random variable $S$, including the event $S<0$. We call this $F$ the random variable’s distribution function.
...
The first important concept to learn is the *expected value*. In statistics books, it is common to use letter $E$
like this:
...
To see this, notice that if there are n beads in the urn, then we have $np as$ and $n(1-p)b$s and because the average is the sum, $n \times a \times p = n \times b \times (1-p)$, divided by the total $n$, we get that the average is $ap + b(1-p)$.
...
$$\sqrt {number \space of \space draws} \times average \space of \space the \space number \space in \space the \space urn$$
...
Using the formula above, the sum of 1,000 people playing has standard error of about $$\text{$}$$32:
...
As a result, when 1,000 people bet on red, the casino is expected to win $$\text{$}$$50 with a standard error of $$\text{$}$$32. It therefore seems like a safe bet. But we still haven’t answered the question: how likely is it to lose money? Here the CLT will help.
...
$$μ = \frac{1}{n}\displaystyle\sum_{i=1}^{n} x_i \\ σ = \sqrt {\frac{1}{n}\displaystyle\sum_{i=1}^{n}(x_i-μ)^2}$$
...
$$X=\frac{1}{N}\displaystyle\sum_{i=1}^{N}X_i,s=\sqrt {\frac{1}{N-1}\displaystyle\sum_{i=1}^{N}(X_i-X)^2}$$
