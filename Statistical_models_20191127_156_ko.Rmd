...
We have 49 estimates of the spread. The theory we learned tells us that these estimates are a random variable with a probability distribution that is approximately normal. The expected value is the election night spread $d$
and the standard error is $2\sqrt{p(1−p)/N}$. Assuming the urn model we described earlier is a good one, we can use this information to construct a confidence interval based on the aggregated data. The estimated spread is:
...
A problem is that we don’t know $\sigma$. But theory tells us that we can estimate the urn model σ with the sample standard deviation defined as 
...
4. So how is this useful? We are going to use an oversimplified yet illustrative example. Suppose we want to know the average height of our male students, but we only get to measure 50 of the 708. We will use $\hat X$ as our estimate. We know from the answer to exercise 3 that the standard estimate of our error $\bar X−μ$ is $σ/\sqrt{N}$. We want to compute this, but we don’t know $σ$. Based on what is described in this section, show your estimate of $σ$.


```{r}
sqrt(sum((sample(x, N, replace = FALSE)- x_sam)^2)/(N-1))
```

5. Now that we have an estimate of $\sigma$, let’s call our estimate $s$. Construct a 95% confidence interval for $μ$.

```{r}
s <- sqrt(sum((sample(x, N, replace = FALSE)- x_sam)^2)/(N-1))

c(x_hat + 1.96 * s, x_hat - 1.96 * s) 
```
...
B <- 10000
N <- 50
simul <- replicate(B, c(x_hat + 1.96 * s, x_hat - 1.96 * s) )

ifelse(mean(x) <simul[1,] & mean(x) > simul[2,], 1)
```
...
```{r}
ggplot(polls, aes(spread, pollster)) + geom_point()
```
...
with $i=1,2$ indexing the two pollsters, $b_i$ the bias for pollster $i$ and $\epsilon_{i,j}$ poll to poll chance variablity. We assume the $\epsilon$ are independent from each other, have expected value 0 and standard deviation $\sigma_i$ regardless of $j$

Which of the following best represents our question?

b. How close are the $Y_{i,j}$ to $d$

9. In the right side of this model only $E_{i,j}$ is a random variable. The other two are constants. What is the expected value of $Y_{i,j}$?

10. Suppose we define $Y_1$ as the average of poll results from the first poll,$Y_{1,1},...Y_{1,N_1}$ the number of polls conducted by the first pollster:
...
12. Suppose we define $\bar Y_2$ as the average of poll results from the first poll, $Y_{2,1},...Y_{2,N_2}$ with the number of polls conducted by the first pollster. What is the expected value $\bar Y_2$?
...
$$\frac {\bar Y_2 - \bar Y_1}{s_2^2/N_2 + s_1^2/N_1}$$
